...
type Launcher struct {
	...
	// defaultTmpMounts is the set of mounts of tmp directories that was
	// generated by default (as opposed to being explicitly requested)
	defaultTmpMounts []specs.Mount
}
...
func (l *Launcher) handleVarTmpToTmpSymlink(spec *specs.Spec) {
	...
	// Remove default mounts whose destination is /var/tmp from the OCI spec
	// we're generating.
	for _, defMount := range vartmpDefaultMounts {
		spec.Mounts = lo.Filter(spec.Mounts, func(m specs.Mount, _ int) bool {
			// The first disjunct here is superfluous, logically speaking; it's
			// here to ensure comparison is as fast as possible. It only makes
			// sense to bother with DeepEqual() if we know the path matches.
			return ((m.Destination != vartmpPath) || !reflect.DeepEqual(m, defMount))
		})
	}
}
