...
func (l *Launcher) getMounts() ([]specs.Mount, error) {
	...
	// To facilitate handleVarTmpToTmpSymlink(), store mounts generated by
	// addTmpMounts() in Launcher in addition to adding them to spec.
	if err := l.addTmpMounts(&l.defaultTmpMounts); err != nil {
		return nil, fmt.Errorf("while configuring tmp mounts: %w", err)
	}
	*mounts = append(*mounts, l.defaultTmpMounts...)
