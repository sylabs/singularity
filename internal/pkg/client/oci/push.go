...
func Push(ctx context.Context, sourceFile string, destRef string, ociAuth *ocitypes.DockerAuthConfig, ociAuthFile string) error {
	...
	switch img.Type {
	case image.OCISIF:
		return ocisif.PushOCISIF(ctx, sourceFile, destRef, ociAuth, ociAuthFile)
	case image.SIF:
